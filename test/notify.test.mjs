...
test('a rule with no when block pings on every item', () => {
  ...
  assert.deepEqual(mentionsFor(item(), notify, NOW), { roles: [ROLE_A], users: [], text: undefined, batching: true, summarize: false });
});
...
test('a rule with a when block pings only on matching items', () => {
  ...
  assert.deepEqual(mentionsFor(item(), notify, NOW), { roles: [ROLE_A], users: [], text: 'New hardware:', batching: true, summarize: false });
...
test('overlapping rules union their targets and dedupe', () => {
  ...
  assert.deepEqual(mentionsFor(item(), notify, NOW), { roles: [ROLE_A, ROLE_B], users: [USER], text: undefined, batching: true, summarize: false });
});
